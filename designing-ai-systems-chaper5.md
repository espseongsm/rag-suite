...
class IndexConfig:
    name: str    #A
    embedding_model: str =
➥"text-embedding-3-small"    #B
    embedding_dimensions: int = 1536    #B
    chunking_strategy: str = "fixed"    #C
    chunk_size: int = 512    #C
    chunk_overlap: int = 50    #C
    metadata_schema: Optional[Dict] = None    #D
...
➥IndexConfig) -> Index:    #A
    ...
    def list_indexes(self) -> List[Index]:    #B
        """Return all indexes the caller has access to."""
        pass
 
    def delete_index
➥(self, index_name: str) -> bool:    #C
...
class Index:
    name: str    #A
    config: IndexConfig    #A
    owner: str = ""    #B
    document_count: int = 0    #C
    total_chunks: int = 0    #C
    created_at: datetime = None    #D
    last_ingested_at: Optional[datetime] = None    #D
...
class DocumentSection:
    content: str
    heading: Optional[str] = None    #A
    level: int = 0    #A
    page_number: Optional[int] = None

@dataclass
class ExtractedDocument:
    sections: List[DocumentSection]    #B
    metadata: Dict[str, str]    #C
...
class IngestionPipeline:
    ...
    def detect_format(self, filename: str, file_bytes: bytes) -> str:
        if file_bytes[:5] == b"%PDF-":    #A
            return "pdf"    #A
        if file_bytes[:4] == b"PK\x03\x04":    #A
            return "docx"    #A
        # ... other magic byte checks for supported formats
 
        extension = filename.rsplit(".", 1)[-1].lower()    #B
...
class DocumentMetadata:
    ...
    page_count: Optional[int] = None    #B
    word_count: Optional[int] = None    #B
    custom_metadata: Dict[str, str] = field(    #C
        default_factory=dict    #C
    )    #C
...
class Chunk:
    text: str    #A
    heading: Optional[str] = None    #B
    start_offset: int = 0    #C
    end_offset: int = 0    #C
...
platform.data.register_chunking_strategy(
    name="faq",    #A
    strategy=FAQChunkingStrategy(),    #B
    version="1.0.0",
)
...
def chunk_fixed_size(
    ...
    while start < len(tokens):
        ...
        chunk_text = detokenize(chunk_tokens)    #B
        chunks.append(Chunk(
            text=chunk_text,
            start_offset=start,
            end_offset=end
        ...
        start += chunk_size - chunk_overlap    #C
...
class EmbeddingGenerator:
    def __init__(self, model_client):    #A
    ...
    def embed_chunks(
        ...
        for i in range(0, len(chunks), batch_size):
            ...
            response = self.model_client.embed(    #C
                texts=texts,
                model=model
            )
...
class IngestJob:
    job_id: str    #A
    status: str    #B
    document_id: Optional[str] = None    #C
    progress: float = 0.0    #D
    error: Optional[str] = None    #E
...
CREATE TABLE chunks (
    ...
    document_id VARCHAR(255) NOT NULL,    #B
    index_name VARCHAR(255) NOT NULL,    #B
    chunk_text TEXT NOT NULL,
    embedding vector(1536),    #C
    metadata JSONB DEFAULT '{}',    #D
...
➥ON chunks USING gin (metadata);    #E
    ...
    def search(
        ...
        if metadata_filters:
            for key, value in metadata_filters.items():
                ...
                params.extend([key, value])    #B
 
        if score_threshold:
            query += " AND 1 - (embedding <=> %s::vector) >= %s"
            params.extend
➥([query_embedding, score_threshold])    #C

        query += " ORDER BY embedding <=> %s::vector LIMIT %s"
        params.extend([query_embedding, top_k])    #D
...
def search(
    ...
    index = self.get_index(index_name)    #A
 
    query_embedding = self.embedding_generator.embed_query(
        query=query,
        model=index.config.embedding_model    #B
    )
...
➥does not support keyword search"    #C
        )    #C
...
CREATE INDEX idx_chunks_search
    ON chunks USING gin (search_vector);    #B
...
class DataClient:
    def __init__(self, channel: grpc.Channel):
        self._stub = DataServiceStub(channel)    #A
...
Chapters
1
Why your AI projects need a platform
2
Building with the platform: SDK and API design
2.1
...
Summary
3
The Model Service: Your platform’s gateway to AI models
4
The Session Service: Teaching your AI to remember
5
The Data Service: teaching AI what your organization knows
5.1
...
Index operations
5.3
Ingestion pipeline: from raw files to vectors
...
Choosing a vector store backend
The pgvector implementation
Search orchestration
5.5
Hybrid search: combining vectors with keywords
Adding keyword search to the platform
PostgreSQL keyword search implementation
...
Summary
6
Tools and Guardrails: Enabling safe, managed AI actions
7
Observability and experimentation: seeing and improving what AI does
